...
def signup():
    if request.method=="POST":
       ...
       # new_user=db.engine.execute(f"INSERT INTO `user` (`srfid`,`email`,`dob`) VALUES ('{srfid}','{email}','{encpassword}') ")
        newuser=User(srfid=srfid,email=email,dob=encpassword)
...
def hospitalUser():
    ...
    if('user' in session and session['user']=="admin"):
        ...
        if request.method=="POST":
            ...
            hosuser=Hospitaluser(hcode=hcode,email=email,password=encpassword)
...
def addhospitalinfo():
    ...
    if request.method=="POST":
        ...
        if huser:            
            ...
            hosdata=Hospitaldata(hcode=hcode,hname=hname,normalbed=nbed,hicubed=hbed,icubed=ibed,vbed=vbed)
...
@app.route("/dischargendelete/<string:srf>",methods=['POST','GET'])
@login_required
def deletendescharge(srf):
    patient=Bookingpatient.query.filter_by(srfid=srf).first()
